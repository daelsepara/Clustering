...
namespace DeepLearnCS
{
	public static class ManagedFile
	{
		...
		public static void Save1DY(string filename, ManagedIntList A)
		{
			using (var file = new StreamWriter(filename, false))
			{
				for (int y = 0; y < A.x; y++)
		...
		public static void Save2D(string filename, ManagedArray A, ManagedIntList B, char delimiter = ',')
		{
			if (A.y != B.x)
				return;
